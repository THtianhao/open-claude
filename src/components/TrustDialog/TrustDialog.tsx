...
export function TrustDialog(t0) {
  ...
  if ($[16] !== hasAnyBashExecution || $[17] !== onDone) {
    t14 = function onChange(value) {
      if (process.env.OPEN_CLAUDE_TRUST_DEBUG === "1") {
        try {
          appendFileSync("/tmp/open-claude-trust-debug.log", `[TrustDialog onChange] ${String(value)}\n`);
        } catch {
          /* ignore */
        }
      }
      ...
      if (process.env.OPEN_CLAUDE_TRUST_DEBUG === "1") {
        try {
          appendFileSync("/tmp/open-claude-trust-debug.log", `[TrustDialog onChange] calling onDone()\n`);
        } catch {
          /* ignore */
        }
      }
  ...
  let t21;
  if ($[25] !== onChange) {
    t21 = <Select defaultFocusValue="enable_all" options={t20} onChange={value_0 => onChange(value_0 as 'enable_all' | 'exit')} onCancel={() => onChange("exit")} />;
    $[25] = onChange;
    $[26] = t21;
  } else {
    t21 = $[26];
