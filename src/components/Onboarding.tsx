...
type StepId = 'preflight' | 'theme' | 'model-setup' | 'oauth' | 'api-key' | 'security' | 'terminal-setup';

const OPENAI_MODEL_OPTIONS = [
  { label: 'gpt-4o          · Best for coding (recommended)', value: 'gpt-4o' },
  { label: 'gpt-4o-mini     · Faster & cheaper', value: 'gpt-4o-mini' },
  { label: 'gpt-4.1         · Latest GPT-4.1', value: 'gpt-4.1' },
  { label: 'gpt-4.1-mini    · GPT-4.1 Mini', value: 'gpt-4.1-mini' },
  { label: 'o4-mini         · Fast reasoning', value: 'o4-mini' },
  { label: 'o3              · Strong reasoning', value: 'o3' },
]

function ModelSetupStep({ onDone }: { onDone(): void }): React.ReactNode {
  const provider = getAPIProvider()
  return (
    <Box flexDirection="column" gap={1} paddingLeft={1}>
      <Text bold>Choose your AI model</Text>
      {provider === 'openai' && (
        <Box flexDirection="column" gap={1} width={70}>
          <Text dimColor>Select the OpenAI model to use. You can change this later with <Text bold>/model openai &lt;name&gt;</Text></Text>
          <Select
            ...
            onCancel={() => {
              // default to gpt-4o-mini if skipped
              if (!process.env.OPENAI_MODEL) {
                process.env.OPENAI_MODEL = 'gpt-4o-mini'
                saveGlobalConfig(c => ({ ...c, openaiModel: 'gpt-4o-mini' }))
              }
              onDone()
            }}
          />
          <Text dimColor>Enter to confirm · Esc to skip (defaults to gpt-4o-mini)</Text>
        </Box>
      )}
    </Box>
  )
...
export function Onboarding({
  ...
  // Model selection step — shown on first run for all users
  steps.push({
    id: 'model-setup',
    component: <ModelSetupStep onDone={goToNextStep} />
  });
