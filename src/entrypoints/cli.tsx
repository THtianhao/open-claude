...
// Force TTY when OPEN_CLAUDE_FORCE_INTERACTIVE=1 (e.g. Cursor embedded terminal)
// Must be set before any module reads process.stdout.isTTY
if (process.env.OPEN_CLAUDE_FORCE_INTERACTIVE === '1') {
  // Force stdout/stderr so isNonInteractive check passes
  for (const stream of [process.stdout, process.stderr] as NodeJS.WriteStream[]) {
    if (!stream.isTTY) {
      Object.defineProperty(stream, 'isTTY', { value: true, writable: true, configurable: true });
    }
  }
  // stdin: force isTTY + inject no-op setRawMode so Ink doesn't crash
  if (!process.stdin.isTTY) {
    Object.defineProperty(process.stdin, 'isTTY', { value: true, writable: true, configurable: true });
  }
  if (!(process.stdin as NodeJS.ReadStream & { setRawMode?: unknown }).setRawMode) {
    (process.stdin as NodeJS.ReadStream & { setRawMode: (mode: boolean) => NodeJS.ReadStream }).setRawMode =
      (_mode: boolean) => process.stdin;
  }
}
