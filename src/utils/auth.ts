...
export function getAnthropicApiKeyWithSource(
  ...
  // Local dev bypass: allow running without login/keychain/oauth.
  // This keeps the CLI boot path alive even when auth is unset.
  if (isEnvTruthy(process.env.CLAUDE_CODE_BYPASS_AUTH)) {
    return {
      key: process.env.ANTHROPIC_API_KEY ?? 'sk-local-bypass',
