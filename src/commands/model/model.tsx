...
function SetOpenAIConfig({
  ...
  React.useEffect(() => {
    if (!value) {
      onDone(`Usage: /model openai <model>  |  /model key <apikey>  |  /model baseurl <url>`, { display: 'system' })
      return
    }
    if (field === 'model') {
      process.env.OPENAI_MODEL = value
      saveGlobalConfig(c => ({ ...c, openaiModel: value }))
      setAppState(prev => ({ ...prev, openaiModel: value }))
      onDone(`OpenAI model set to ${chalk.bold(value)}`)
    } else if (field === 'key') {
      process.env.OPENAI_API_KEY = value
      saveGlobalConfig(c => ({ ...c, openaiApiKey: value }))
      setAppState(prev => ({ ...prev, openaiApiKey: value }))
      onDone(`OpenAI API key updated`)
    } else if (field === 'baseurl') {
      process.env.OPENAI_BASE_URL = value
      saveGlobalConfig(c => ({ ...c, openaiBaseUrl: value }))
      setAppState(prev => ({ ...prev, openaiBaseUrl: value }))
      onDone(`OpenAI base URL set to ${chalk.bold(value)}`)
    }
  }, [])
  return null
}
...
function ShowOpenAIStatus({
  ...
  const openaiModel = useAppState(s => s.openaiModel)
  const openaiBaseUrl = useAppState(s => s.openaiBaseUrl)
  const hasKey = !!(useAppState(s => s.openaiApiKey) || process.env.OPENAI_API_KEY || getGlobalConfig().openaiApiKey)
  const model = openaiModel || process.env.OPENAI_MODEL || getGlobalConfig().openaiModel || 'gpt-4o-mini (default)'
  const baseUrl = openaiBaseUrl || process.env.OPENAI_BASE_URL || getGlobalConfig().openaiBaseUrl || 'https://api.openai.com/v1 (default)'
  onDone(`OpenAI model: ${chalk.bold(model)}\nOpenAI base URL: ${baseUrl}\nAPI key: ${hasKey ? chalk.green('set') : chalk.red('not set')}`)
  return null
}
...
export const call: LocalJSXCommandCall = async (onDone, _context, args) => {
  ...
  if (COMMON_INFO_ARGS.includes(args)) {
    ...
    if (getAPIProvider() === 'openai') {
      return <ShowOpenAIStatus onDone={onDone} />
    }
    return <ShowModelAndClose onDone={onDone} />;
  }
  if (COMMON_HELP_ARGS.includes(args)) {
    onDone('Run /model to open the model selection menu, or /model [modelName] to set the model.\nOpenAI users: /model openai <model>  |  /model key <apikey>  |  /model baseurl <url>', {
      display: 'system'
    });
    return;
  }

  // OpenAI provider subcommands
  if (args.startsWith('openai ')) {
    return <SetOpenAIConfig field="model" value={args.slice(7).trim()} onDone={onDone} />
  }
  if (args.startsWith('key ')) {
    return <SetOpenAIConfig field="key" value={args.slice(4).trim()} onDone={onDone} />
  }
  if (args.startsWith('baseurl ')) {
    return <SetOpenAIConfig field="baseurl" value={args.slice(8).trim()} onDone={onDone} />
  }

  // For OpenAI provider, show status instead of the Claude model picker
  if (!args && getAPIProvider() === 'openai') {
    return <ShowOpenAIStatus onDone={onDone} />
  }
